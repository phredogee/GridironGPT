...
def _render_score_rows(scores, *, expanded_count: int = 5) -> None:
    for rank, score in enumerate(scores, start=1):
        explanation_service_item = None
        # Overall rankings already carry rank-aware explanations. Position views
        # reuse the score evidence directly and keep the UI compact.
        header = (
...
def _render_overall_rows(explained_overall, *, limit: int) -> None:
...
def render_fantasy_rankings() -> None:
    ...
    try:
        # Build the full eligible population once so position lists are not
        # accidentally constrained by the selected overall display limit.
    ...
    st.caption(
        "PDF uses compact draft-day takeaways; Excel includes full components "
    ...
    for tab, position in zip(tabs[1:], POSITIONS):
        with tab:
            ...
            _render_score_rows(scores[:position_limit])
