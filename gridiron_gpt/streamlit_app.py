...
def _render_advisor_result(question: str, response: dict, ranked) -> None:
    matched = _advisor_match(question, ranked)
    st.markdown("### Cortex Analysis")
    if matched is None:
        st.write(response["answer"])
        render_confidence_panel(response["confidence"], [])
        if response.get("details"):
            with st.expander("Analysis details", expanded=False):
                for detail in response["details"]:
                    st.write(f"• {detail}")
        return
    player, team, data = matched
    ...
    col2.metric("Cortex Score", f"{score:+.2f}")
    col3.metric("Signals", len(signals))
    st.write(response["answer"])
    render_confidence_panel(response["confidence"], signals)
    st.markdown("### Why Cortex recommends this")
    if signals:
        ranked_signals = sorted(signals, key=lambda signal: abs(float(signal.get("value", 0.0))), reverse=True)
        for signal in ranked_signals[:4]:
            value = float(signal.get("value", 0.0))
            direction = "Positive" if value > 0 else "Negative" if value < 0 else "Neutral"
            st.write(f"• **{direction} {value:+.2f}** — {signal.get('headline', 'Signal')}")
    else:
        st.info("No scored supporting signals are available yet.")
    render_signal_breakdown(signals)
    _render_intelligence_bars(data)
    ...
    if signals:
        for signal in reversed(signals[-5:]):
            ...
            st.caption(f"{signal.get('source', 'Unknown source')} · {signal.get('impact', 'unknown')} · {float(signal.get('value', 0.0)):+.2f}")
    else:
        st.info("No supporting headlines are available yet.")
    with st.expander("Developer / Cortex workflow", expanded=False):
        for detail in response.get("details", []):
            st.write(f"• {detail}")
        st.divider()
        for step in ("Interpret question", "Identify players/teams", "Search scorecards", "Expand knowledge graph", "Evaluate propagation", "Generate recommendation", "Explain reasoning"):
            st.write(f"• {step}")
...
positions = {item["player"]: item.get("position", "UNK") for item in catalog}
scores = apply_adjusted_scores(calculate_player_scores())
ranked_players = sorted(scores.items(), key=lambda item: item[1].get("adjusted_score", item[1]["score"]), reverse=True)
ranked_players = [((player, team), data) for (player, team), data in ranked_players if data["score"] != 0]
...
cortex = CortexFacade()
buy_players = [((player, team), data) for (player, team), data in ranked_players if recommendation_from_score(data.get("adjusted_score", data["score"])) == "BUY"]
watch_players = [((player, team), data) for (player, team), data in ranked_players if recommendation_from_score(data.get("adjusted_score", data["score"])) == "WATCH"]
risk_players = [((player, team), data) for (player, team), data in ranked_players if recommendation_from_score(data.get("adjusted_score", data["score"])) in ["MONITOR", "SELL"]]
version = get_project_version()
selected_page = render_sidebar(version=version)
page_metadata = NAVIGATION_ITEMS[selected_page]
render_shell_header(page_name=page_metadata["label"], description=page_metadata["description"])
st.divider()
...
if selected_page == "Dashboard":
    dashboard = build_dashboard_view_model(ranked_players=ranked_players, buy_players=buy_players, watch_players=watch_players, risk_players=risk_players, player_count=len(player_names), recommendation_from_score=recommendation_from_score, confidence_from_signals=confidence_from_signals, passing_tests=619)
    render_dashboard(dashboard, scores=scores, positions=positions)
if selected_page == "Advisor":
    st.markdown("### Ask Cortex")
    st.caption("Ask football questions in natural language.")
    question = st.text_area("Ask Gridiron Cortex", placeholder="Examples:\n• Who should I start this week?\n• Best waiver pickup over the next 3 weeks?\n• Should I trade Tank Dell?\n• Best DST to stream next week?", height=140)
    if st.button("Ask Cortex", use_container_width=True):
        if question.strip():
            _render_advisor_result(question, roster_advisor.answer(question), ranked_players)
        else:
...
if selected_page == "Players":
    ...
    if intel["status"] == "not_found":
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("Recommendation", intel["recommendation"])
        col2.metric("Score", f"{intel['score']:+.2f}")
        col3.metric("Confidence", f"{intel['confidence']}%")
        st.divider()
        ...
        if momentum.get("status") == "first_snapshot": st.info("First snapshot recorded. More history is needed for momentum.")
            ...
            st.metric("Momentum Score", f"{momentum.get('momentum_score', 0):+.2f}", delta=momentum.get("direction", "stable"))
        ...
        if trend.get("status") == "first_snapshot": st.info("First trend snapshot recorded. More history is needed.")
            ...
            st.write(f"Current Score: `{trend.get('current_score')}`"); st.write(f"Previous Score: `{trend.get('previous_score')}`"); st.write(f"Change: `{trend.get('change'):+.2f}`"); st.write(f"Direction: **{trend.get('direction', 'stable').upper()}**")
...
if selected_page == "Trends":
    ...
    hot_players.sort(key=lambda item: item["velocity"], reverse=True); cold_players.sort(key=lambda item: item["velocity"])
