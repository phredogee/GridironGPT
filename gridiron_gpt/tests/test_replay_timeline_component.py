...
from apps.streamlit.components.replay_timeline import build_replay_snapshot, replay_option_label, replay_position, stage_card_fields
...
def test_stage_card_fields_extract_signal_intelligence():
    step = _step(ReplayStage.UNDERSTOOD, CortexEventType.SIGNAL_CREATED, "signal", {"signal_category": "opportunity", "impact_score": 0.75, "confidence": 0.88})
    assert stage_card_fields(step) == (("Type", "Opportunity"), ("Impact", "+0.750"), ("Signal Confidence", "88%"))

def test_stage_card_fields_extract_propagation_intelligence():
    step = _step(ReplayStage.PROPAGATED, CortexEventType.PROPAGATION_COMPLETED, "propagated", {"impact_count": 2, "impacts": [{"entity_name": "Nico Collins", "relationship_type": "teammate", "propagation_weight": 0.35, "projected_impact": 0.21}]})
    assert stage_card_fields(step) == (("Impacts", "2"), ("Target", "Nico Collins"), ("Relationship", "Teammate"), ("Weight", "+0.350"), ("Projected Impact", "+0.210"))

def test_stage_card_fields_extract_score_and_recommendation():
    score = _step(ReplayStage.SCORED, CortexEventType.SCORE_UPDATED, "score", {"previous_score": 50.0, "new_score": 51.2, "score_delta": 1.2})
    rec = _step(ReplayStage.RECOMMENDED, CortexEventType.RECOMMENDATION_CHANGED, "rec", {"previous_recommendation": "HOLD", "recommendation": "BUY", "confidence": 0.91})
    assert ("Delta", "+1.200") in stage_card_fields(score); assert stage_card_fields(rec) == (("Previous", "HOLD"), ("Recommendation", "BUY"), ("Confidence", "91%"))
