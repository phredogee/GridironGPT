...
def empty_projection_loader(*, scoring):
    return {}
...
def test_build_loads_role_snapshot_when_explicit_role_scores_are_absent():
    ...
    def historical_loader(*, scoring):
        return pd.DataFrame([{"player_display_name": "Player One", "hist_score": 100.0}])
    def adp_loader(*, scoring, teams):
        return AdpSnapshot(records={"Player One": {"adp": 10.0}}, year=2026)
...
def test_projection_loader_feeds_projected_points_into_population():
    population_service = StubPopulationService()
    def historical_loader(*, scoring): return pd.DataFrame()
    def adp_loader(*, scoring, teams): return AdpSnapshot(records={}, year=None)
    def projection_loader(*, scoring):
        assert scoring == "ppr"
        return {
            "Player One": SimpleNamespace(projected_points=321.5),
            "Player Two": SimpleNamespace(projected_points=250.0),
        }
