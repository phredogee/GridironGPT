...
def event(event_type, minute, *, correlation="run-1", entity="Tank Dell", payload=None):
    return CortexEvent(
        ...
        source="ESPN",
        payload=payload or {},
        event_id=f"{correlation}-{minute}-{event_type.value}",
    )
...
def complete_events(correlation="run-1"):
    return (
        ...
        event(CortexEventType.PLAYER_RESOLVED, 1, correlation=correlation, payload={"confidence": 0.98}),
...
def test_replay_decision_id_is_stable():
    first = build_replay_decision(complete_events())
    second = build_replay_decision(complete_events())
...
def test_replay_engine_returns_latest_and_by_player():
    bus = CortexEventBus()
    bus.publish_many(complete_events("older"))
    newer = tuple(
        CortexEvent(
            event_type=item.event_type,
            timestamp=item.timestamp + timedelta(hours=1),
            correlation_id="newer",
            entity_id=item.entity_id,
            entity_name="Jahmyr Gibbs",
            source=item.source,
            payload=dict(item.payload),
            event_id=f"newer-{item.event_id}",
        )
        for item in complete_events("template")
    )
    bus.publish_many(newer)
...
def test_replay_engine_finds_decision_and_handles_empty_history():
    ...
    bus = CortexEventBus()
    bus.publish_many(complete_events())
    replay = ReplayEngine(bus)
    decision = replay.by_correlation("run-1")
