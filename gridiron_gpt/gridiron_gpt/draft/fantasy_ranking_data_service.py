...
from gridiron_gpt.draft.fantasy_ranking_tier_service import FantasyRankingMarketView, FantasyRankingTierService
...
class FantasyRankingDataService:
    ...
    def __init__(self, population_service: FantasyRankingPopulationService, *, historical_loader: Callable = get_historical_scores, adp_loader: Callable = fetch_adp, role_loader: Callable | None = None, projection_loader: Callable | None = None, adp_source_loaders: dict[str, Callable] | None = None, consensus_adp_service: ConsensusAdpService | None = None, tier_service: FantasyRankingTierService | None = None, ranking_season: int = 2026) -> None:
        ...
        self.adp_source_loaders = self._default_runtime_adp_source_loaders(season=ranking_season) if adp_source_loaders is None else dict(adp_source_loaders)
    ...
    def build(self, *, scoring: str = "ppr", teams: int = 12, limit: int | None = None, role_scores_by_player_id: dict[str, float] | None = None, role_provenance_by_player_id: dict[str, str] | None = None) -> FantasyRankingDataSnapshot:
        ...
        try:
            ...
            projections = {}
        projected_points_by_name: dict[str, float] = {}
        for name, projection in (projections or {}).items():
            fantasy = getattr(projection, "fantasy", None)
            raw_points = getattr(fantasy, "projected_points", None)
            points = self._finite_nonnegative(raw_points)
            if points is not None:
                projected_points_by_name[str(name)] = points
        ...
        adp_by_name = {record.player_name: record.consensus_adp for record in consensus_adp_by_key.values()}
        draft_pool_size = min(len(adp_by_name), self.MARKET_DRAFT_POOL_SIZE) if adp_by_name else None
        ...
        population = self.population_service.build(historical_points_by_name=historical_points, adp_by_name=adp_by_name, role_scores_by_player_id=role_scores_by_player_id, role_provenance_by_player_id=role_provenance_by_player_id, projected_points_by_name=projected_points_by_name, draft_pool_size=draft_pool_size, limit=limit)
        market_views = self.tier_service.build(population.overall, consensus_adp_by_key=consensus_adp_by_key)
        source_names = tuple(sorted(sources))
        adp_year = self.ranking_season if adp_by_name else primary_snapshot.year
        return FantasyRankingDataSnapshot(population=population, historical_player_count=len(historical_points), adp_player_count=len(adp_by_name), adp_year=adp_year, adp_used=bool(adp_by_name), role_player_count=len(role_scores_by_player_id or {}), role_season=role_snapshot.season, consensus_adp_by_key=consensus_adp_by_key, market_views_by_player_id=market_views, adp_sources=source_names, projection_player_count=len(projected_points_by_name))
    ...
    def _adp_sources(self, primary_snapshot: AdpSnapshot) -> dict[str, dict[str, float]]:
        ...
        if primary_snapshot.year == self.ranking_season:
            primary_values = {name: float(record["adp"]) for name, record in primary_snapshot.records.items() if record.get("adp") is not None and self._finite_positive(record.get("adp")) is not None}
            if primary_values:
    ...
    def _load_adp(self, *, scoring: str, teams: int) -> AdpSnapshot:
        ...
        return result if isinstance(result, AdpSnapshot) else AdpSnapshot(records=result or {}, year=None)
    ...
    def _load_observed_role_snapshot(*, season: int) -> RoleSnapshot:
        try:
            ...
            if hasattr(frame, "to_pandas"): frame = frame.to_pandas()
        except Exception:
            return RoleSnapshot({}, {}, None)
        if frame is None or getattr(frame, "empty", True): return RoleSnapshot({}, {}, None)
        required = {"player_id", "position", "week"}
        if not required.issubset(frame.columns): return RoleSnapshot({}, {}, None)
        team_col = "recent_team" if "recent_team" in frame.columns else "team"
        if team_col not in frame.columns: return RoleSnapshot({}, {}, None)
        work = frame.copy()
        work = work[work["position"].isin(["QB", "RB", "WR", "TE"])]
        work = work[work["player_id"].notna()]
        if work.empty: return RoleSnapshot({}, {}, None)
        max_week = work["week"].max()
        work = work[work["week"] >= max(1, int(max_week) - 5)].copy()
        for column in ("attempts", "carries", "targets"):
            if column not in work.columns: work[column] = 0.0
        ...
        def safe_share(numerator, denominator): return numerator.where(denominator > 0, 0.0) / denominator.where(denominator > 0, 1.0)
        ...
        qb = work["position"] == "QB"; rb = work["position"] == "RB"; receiver = work["position"].isin(["WR", "TE"])
        work.loc[qb, "role_metric"] = work.loc[qb, "attempt_share"]
        work.loc[rb, "role_metric"] = (work.loc[rb, "carry_share"] + work.loc[rb, "target_share_calc"]) / 2.0
        work.loc[receiver, "role_metric"] = work.loc[receiver, "target_share_calc"]
        summary = work.groupby(["player_id", "position"], as_index=False).agg(role_metric=("role_metric", "mean"))
        summary["role_score"] = summary.groupby("position")["role_metric"].rank(method="average", pct=True).mul(100.0)
        scores: dict[str, float] = {}; provenance: dict[str, str] = {}
        for row in summary.itertuples(index=False):
            ...
            provenance[player_id] = f"{season} recent observed usage percentile for {row.position} (last six available weeks)"
    ...
    def _historical_points_by_name(frame) -> dict[str, float]:
        ...
        for row in frame[["player_display_name", "hist_score"]].itertuples(index=False):
            ...
            try: score = float(row.hist_score)
            except (TypeError, ValueError): continue
            if not math.isfinite(score): continue
            previous = values.get(name)
            if previous is None or score > previous: values[name] = score
        return values
