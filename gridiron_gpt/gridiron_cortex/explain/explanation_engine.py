...
class ExplanationEngine:
    ...
    def explain(
        ...
        for rec in recommendations:
            ...
            prediction = predictions_by_name.get(
                rec.entity_name.strip().casefold()
            )
    ...
    def build_evidence_chains(
        ...
        """Build one structured reasoning chain per recommendation."""
        ...
        for recommendation in recommendations:
            ...
            steps = [
                EvidenceStep(
                    ...
                    value=signal.impact_score,
                    reasons=self._signal_reasons(signal),
                ),
                EvidenceStep(
                    faculty="Understand",
                    step_type="signal",
                    summary=(
                        f"Classified as {signal.sentiment} "
            ...
            entity_impacts = impacts_by_name.get(key, [])
            ...
            steps.append(
                EvidenceStep(
                    faculty="Evaluate",
                    step_type="score_change",
                    summary=(
                        f"Score changed by "
                        f"{recommendation.score_delta:+.3f}"
                    ),
                    entity_name=recommendation.entity_name,
                    value=recommendation.score_delta,
                    reasons=[
                        reason
                        for reason in recommendation.reasons
                        if "forecast:" not in reason.casefold()
                    ],
                )
            )
            ...
            if prediction is not None:
                steps.append(
                    EvidenceStep(
                        ...
                        step_type="forecast",
                        summary=(
                            f"{prediction.horizon_days}-day outlook: "
                            f"{prediction.projected_trend}"
                        ),
                        entity_name=prediction.entity_name,
                        value=prediction.projected_score,
    ...
    def build_evidence_graphs(
        ...
        """Build one causal evidence graph per recommendation."""
        ...
        for recommendation in recommendations:
            ...
            graphs.append(
                EvidenceGraph(
                    ...
                    action=recommendation.action,
                    confidence=recommendation.confidence,
                    root_node_ids=[observe_id],
                    terminal_node_ids=[decide_id],
                    nodes=nodes,
                )
            )
    ...
    def _signal_reasons(signal) -> list[str]:
        ...
        reasons.extend(
            f"Positive indicator: {hit}"
            for hit in signal.positive_hits
        )
        reasons.extend(
            f"Negative indicator: {hit}"
            for hit in signal.negative_hits
        )
